...
const TEMPLATE_FILE = resolve(import.meta.dirname ?? dirname(new URL(import.meta.url).pathname), "run-task-template.mjs");

function ensureRunnerScript(piBin: string): void {
	mkdirSync(SCHEDULER_DIR, { recursive: true });
	const PROMPT_FILE_THRESHOLD = 4000;

	let script = readFileSync(TEMPLATE_FILE, "utf-8");
	script = script.replace("{{PROMPT_FILE_THRESHOLD}}", String(PROMPT_FILE_THRESHOLD));
	script = script.replace("{{PI_BIN}}", JSON.stringify(piBin));
